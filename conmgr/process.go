...
func StartProcess(c *model.Container) error {
	errstr := `参数缺一不可，参数格式:{"header":{"token":""},"body":{"params":{"perform":1,"title":"张三-6月一线考核","templateId":"002d5df2a737dd36a2e78314b07d0bb1_1591669930","business_type":"月度考核","data":{}}}} perform:1表示保存先不提交,默认值0表示直接提交`
	// 参数检查
	// 判断token
	...
	if c.Body.Params["business_type"] == nil || len(c.Body.Params["business_type"].(string)) == 0 {
		return errors.New(errstr)
	}
	...
	businessType = c.Body.Params["business_type"].(string)
	...
	if commit {
		...
		err := CompleteProcessByToken(c.Header.Token, c.Body.Params)
		if err != nil {
...
func CompleteProcess(c *model.Container) error {
	errstr := `参数格式:{"header":{"token":""},"body":{"params":{"business_type":"月度考核","data":{"processInstanceId":"B4MttX5xHnfKdf"},"perform":2,"speech":""}}}`
	if c.Body.Params["business_type"] == nil || len(c.Body.Params["business_type"].(string)) == 0 {
		return fmt.Errorf("business_type不能为空,%s", errstr)
	}
	// 检测远程用户端口是否可用
	ok := IsUserAPIAlive()
	if !ok {
	...
	if len(data) > 0 {
		err := FirstOrCreateFlowData(c.Body.Params["business_type"].(string), data)
		if err != nil {
